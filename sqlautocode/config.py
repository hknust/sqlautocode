...
def _version_check(parser):
    ...
    #     parser.error("SQLAlchemy version 0.4.0 or higher is required. (%s)" % ex)
...
def _set_output(path, overwrite=False):
    if os.path.exists(path) and not overwrite:
        print('File "%s" exists and will be overwritten.' % path, file=sys.stderr)
    ...
    try:
        ...
        print('Could not open "%s" for writing: %s' % (path, e), file=sys.stderr)
        sys.exit(-1)
